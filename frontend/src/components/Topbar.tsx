    ...
    <header className="h-16 bg-[#F7F9FC]/95 backdrop-blur-md flex items-center justify-between px-3 sm:px-6 lg:px-8 fixed top-0 right-0 left-0 md:left-64 z-20 border-b border-[#E6ECEB]/80 shadow-2xs">
      ...
      <div className="flex items-center gap-2 flex-1 max-w-sm">
        ...
        <div className="relative w-full max-w-[140px] sm:max-w-xs">
          ...
          <input
            ...
            className="w-full bg-white border border-[#E6ECEB] rounded-full pl-9 pr-3 py-1.5 sm:py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#00694c]/20 focus:border-[#00694c] shadow-2xs font-medium"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[#E6ECEB] rounded-full text-xs text-slate-700 shadow-2xs">
          <span className="h-2 w-2 rounded-full bg-[#00694c]"></span>
          <span className="text-slate-600 font-medium">Employee</span>
          <span className="font-extrabold text-slate-900">{user?.id_number || 'EMP002'}</span>
        </div>
        ...
        <div className="relative">
          <button 
            ...
            className="h-9 w-9 sm:h-10 sm:w-10 rounded-xl bg-white border border-[#E6ECEB] flex items-center justify-center text-slate-600 hover:text-[#00694c] hover:border-[#00694c]/40 transition shadow-2xs relative cursor-pointer"
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-3 w-[calc(100vw-2rem)] max-w-xs sm:w-80 bg-white border border-[#E6ECEB] rounded-2xl shadow-xl py-2 z-50">
              <div className="px-4 py-2.5 border-b border-slate-100 flex justify-between items-center bg-[#e6f4ee]/30">
                ...
                {notifications.length > 0 && (
                  <button 
                    ...
                    className="text-[10px] text-[#00694c] font-bold hover:underline cursor-pointer"
                  >
                    Clear All
                  </button>
                )}
        ...
        {/* Circular Mail Icon */}
        <div className="hidden sm:flex h-10 w-10 rounded-xl bg-white border border-[#E6ECEB] items-center justify-center text-slate-600 hover:text-[#00694c] hover:border-[#00694c]/40 transition shadow-2xs cursor-pointer">
        ...
        {user && (
          <div className="flex items-center gap-2 sm:gap-2.5 bg-white border border-[#E6ECEB] rounded-full py-1 sm:py-1.5 pl-1.5 sm:pl-2 pr-2.5 sm:pr-3 shadow-2xs">
            <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-xl bg-[#00694c] text-white font-black flex items-center justify-center text-xs shadow-2xs shrink-0">
              {user.name.charAt(0)}
            </div>
            <div className="hidden sm:block text-left">
              <div className="text-xs font-black text-slate-900 leading-tight truncate max-w-[100px]">{user.name}</div>
              <div className="text-[10px] font-medium text-slate-400 truncate max-w-[100px]">{user.role}</div>
            </div>
            <button
              onClick={onLogout}
              title="Logout"
              className="text-slate-400 hover:text-[#00694c] transition cursor-pointer p-0.5"
            >
